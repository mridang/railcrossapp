...
let SchedulerService = (SchedulerService_1 = class SchedulerService {
  ...
  constructor(
    scheduler = new client_scheduler_1.SchedulerClient(),
    group = constants_1.scheduleGroup,
    schedulerRoleArn = `arn:aws:iam::${process.env.ACCOUNT_ID}:role/${constants_1.roleName}`,
  ) {
  ...
  async updateSchedules(repoName, lockTime, unlockTime, timeZone) {
    const schedules = await this.scheduler.send(
      new client_scheduler_1.ListSchedulesCommand({
        GroupName: this.group,
    ...
    for (const schedule of schedules.Schedules || []) {
      ...
      if (schedule.Name === undefined) {
        ...
        await this.scheduler.send(
          new client_scheduler_1.UpdateScheduleCommand({
  ...
  async deleteSchedules(repoName) {
    ...
    for (const schedule of schedules.Schedules || []) {
      ...
      await this.scheduler.send(
        new client_scheduler_1.DeleteScheduleCommand({
          Name: schedule.Name,
          GroupName: this.group,
        }),
      );
    ...
    return await Promise.all(
      (schedules.Schedules || []).map(async (schedule) => {
        return await this.scheduler.send(
          new client_scheduler_1.GetScheduleCommand({
            Name: schedule.Name,
          }),
        );
      }),
...
SchedulerService = SchedulerService_1 = __decorate(
  [
    ...
    __metadata('design:paramtypes', [
      client_scheduler_1.SchedulerClient,
      String,
      String,
    ]),
  ],
